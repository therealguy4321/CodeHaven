...
def history():
    ...
    # Check database for user's transactions, ordered by most recent first
    transactions = db.execute(
        "SELECT * FROM transactions WHERE user_id = :user_id ORDER BY timestamp DESC",
        user_id=session["user_id"],
    )

    # Iterate over transactions and add formatted price and total
    for transaction in transactions:
        symbol = transaction["symbol"]
        quote = lookup(symbol)
        if quote is not None:
            transaction["price_formatted"] = usd(quote["price"])
            transaction["total_formatted"] = usd(
                transaction["price"] * transaction["shares"]
            )
        else:
            transaction["price_formatted"] = "N/A"
            transaction["total_formatted"] = "N/A"
...
def sell():
    ...
    if request.method == "POST":
        ...
        # Check if the symbol and shares are valid
        if not symbol:
            return apology("Must provide symbol")
        elif not shares_to_sell or shares_to_sell <= 0:
            return apology("Must provide a positive integer number of shares")

        # Get user's stocks
        user_stocks = db.execute(
            "SELECT symbol, SUM(shares) as total_shares FROM transactions WHERE user_id = :user_id AND symbol = :symbol GROUP BY symbol",
            user_id=user_id,
            symbol=symbol,
        )

        # Check if the user owns the specified symbol and has enough shares
        ...
        # Update users table (increase cash)
        db.execute(
        ...
        # Add the sale to the history table (decrease shares)
        db.execute(
            "INSERT INTO transactions (user_id, symbol, shares, price) VALUES (:user_id, :symbol, :shares, :price)",
            user_id=user_id,
            symbol=symbol,
        ...
        # Get user's symbols for selling
        user_id = session["user_id"]
